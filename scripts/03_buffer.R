## -------------------------------------------------------------------------------------------------------
# 03_buffer.R
# Create a Spatial Buffer Script to identify 1/3 of 1990 tracts to keep 
# and identify 2/3 of 2000 tracts to keep as a part of validation procedure mimicking missing tracts
# author: tjh
...
### ------------------------------------------------------------------------------
## counties included in validation
county_fips <- c("G2200710", "G2601630", "G2905100",
                 "G0600650", "G1200950","G4804390",
                 "G3400130", "G3900610", "G4200030")

# read in shapefiles (created in 02_pull_in_shapefiles.R)
tracts90_sf <- read_sf("gis_files/input/tracts_1990/tracts90.shp") %>%
  mutate(COUNTYA = str_sub(GISJOIN,1,8))
tracts00_sf <- read_sf("gis_files/input/tracts_2000/tracts00.shp") %>%
  mutate(COUNTYA = str_sub(GISJOIN,1,8))


## create centroids for nine validation counties
##--------------------------------------------------------------------------------------
...
fct_count(tracts90_all$stay)
1564/2344
780/2344

# write csv
write_csv(tracts90_all, "tables/tracts90_1_3_stay.csv")
...
##--------------------------------------------------------------------------------------
## 2/3 tracts to keep, 2000
## retained ._ctr for counties centroids
##--------------------------------------------------------------------------------------
...
wayne_buf <- st_buffer(wayne_ctr, 15500)
wayne_buf_intersects <- st_intersects(wayne_buf, wayne_sf)
wayne_sel_sf <- wayne_sf[wayne_buf_intersects[[1]],]
...
stl_buf <- st_buffer(stl_ctr, 5500)
stl_buf_intersects <- st_intersects(stl_buf, stl_sf)
stl_sel_sf <- stl_sf[stl_buf_intersects[[1]],]
...
write_csv(tracts00_all, "tables/tracts00_2_3_stay.csv")
