...
##--------------------------------------------------------------------------------------
## read in shapefiles
tracts10_sf <- read_sf("gis_files/input/tracts_2010/tracts10.shp")
tracts00_sf <- read_sf("gis_files/input/tracts_2000/tracts00.shp")
tracts90_sf <- read_sf("gis_files/input/tracts_1990/tracts90.shp")
bgs10_sf <- read_sf("gis_files/input/bgroups_2010/bgs10.shp")


##--------------------------------------------------------------------------------------
## read in tracts to remove
remove_tracts_2000 <- read_csv("csv/tracts00_2_3_stay.csv") %>% ## read in file with 1/2 of tracts to stay and 1/2 to remove (created in buf_2000.R)
  select(GISJOIN, stay)
remove_tracts_1990 <- read_csv("csv/tracts90_1_3_stay.csv") %>% # read in file with 1/4 of tracts to stay and 3/4 to remove (created in buf_1990.R)
  select(GISJOIN, stay)
