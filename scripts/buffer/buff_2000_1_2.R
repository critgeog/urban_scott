## --------------------------------------------------------------------------
# buf_2000_1_2.R
# identify 2/3 of 2000 tracts to keep for 2000 data
# author: tjh
...
# read in 2000 census tract file, sf
...
orleans_buf <- st_buffer(orleans_ctr, 4300)
...
wayne_buf <- st_buffer(wayne_ctr, 11800)
...
stl_buf <- st_buffer(stl_ctr, 4200)
...
riverside_buf <- st_buffer(riverside_ctr, 23500)
...
orange_buf <- st_buffer(orange_ctr, 8900)
...
tarrant_buf <- st_buffer(tarrant_ctr, 12500)
...
essex_buf <- st_buffer(essex_ctr, 3650)
...
allegheny_buf <- st_buffer(allegheny_ctr, 8600)
...
hamilton_buf <- st_buffer(hamilton_ctr, 7250)
...
tracts_2000_all <- left_join(tract_2000_sf, all_sel_sf) %>%
  mutate(stay = replace_na(stay,"N"))

tracts_2000_all$stay <- factor(tracts_2000_all$stay, levels = c('Y','N'))

fct_count(tracts_2000_all$stay)

# write_csv(tracts_2000_all, "tracts_2000_1_2_remove.csv")
